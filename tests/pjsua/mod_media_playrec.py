...
def test_func(t):
    ...
    # Get input file name
    input_filename = (
        re.compile(const.MEDIA_PLAY_FILE).search(endpt.inst_param.arg).group(1)
    )
    endpt.trace("Input file = " + input_filename)

    # Get output file name
    output_filename = (
        re.compile(const.MEDIA_REC_FILE).search(endpt.inst_param.arg).group(1)
    )
...
def post_func(t):
    ...
    fullcmd = (
        COMPARE_WAV_EXE + " " + input_filename + " " + output_filename + " " + "3000"
    )

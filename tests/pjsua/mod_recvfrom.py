...
def test_func(test):
    ...
    for t in cfg_file.recvfrom_cfg.transaction:
        ...
        if t.check_cseq:
            ...
            while (
                cseq <= last_cseq and method == last_method and call_id == last_call_id
            ):
...
test = TestParam(
    cfg_file.recvfrom_cfg.name, [cfg_file.recvfrom_cfg.inst_param], test_func
)

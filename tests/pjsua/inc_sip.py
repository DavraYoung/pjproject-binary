...
class Dialog:
    ...
    def __init__(self, dst_addr, dst_port=5060, tcp=False, trace=True, local_port=0):
        ...
        self.trace(
            "Dialog socket bound to " + self.local_ip + ":" + str(self.local_port)
        )
    ...
    def update_fields(self, msg):
        ...
        msg = msg.replace(
            "$TARGET_URI",
            "sip:" + self.dst_addr + ":" + str(self.dst_port) + transport_param,
        )
    ...
    def create_ack(self, sdp="", extra_headers=""):
        return self.create_req(
            "ACK", sdp, extra_headers=extra_headers, branch=self.inv_branch
        )
    ...
    def wait_msg_from(self, timeout):
        ...
        while time.time() < endtime:
            ...
            if len(readset[0]) < 1 or not self.sock in readset[0]:
                if len(readset[0]) < 1:
                    print(
                        "select() timeout (will wait for "
                        + str(int(endtime - time.time()))
                        + "more secs)"
                    )
    ...
    def hangup(self, last_code=0):
        ...
        if self.last_resp_code > 0 and self.last_resp_code < 200:
            msg = self.create_req(
                "CANCEL", "", branch=self.inv_branch, extra_headers=""
            )

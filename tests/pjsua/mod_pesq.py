...
def test_func(t):
    ...
    # Get input file name
    input_filename = (
        re.compile(const.MEDIA_PLAY_FILE).search(ua1.inst_param.arg).group(1)
    )

    # Get output file name
    output_filename = (
        re.compile(const.MEDIA_REC_FILE).search(ua2.inst_param.arg).group(1)
    )
    ...
    if mo_clock_rate == None:
        raise TestError(
            "Cannot compare input & output, incorrect output filename format"
        )
    ...
    if (clock_rate != "8") & (clock_rate != "16"):
        raise TestError(
            "PESQ only works on clock rate 8kHz or 16kHz, clock rate used = "
            + clock_rate
            + "kHz"
        )
...
def post_func(t):
    ...
    if float(pesq_res) >= threshold:
        endpt.trace(
            "Success, PESQ result = " + pesq_res + " (target=" + str(threshold) + ")."
        )
    else:
        endpt.trace(
            "Failed, PESQ result = " + pesq_res + " (target=" + str(threshold) + ")."
        )
        # Save the wav file
        wavoutname = ARGS[1]

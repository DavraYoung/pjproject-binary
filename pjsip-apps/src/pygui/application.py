...
class Application(ttk.Frame):
    ...
    def start(self, cfg_file="pygui.js"):
        ...
        # Initialize library
        self.appConfig.epConfig.uaConfig.userAgent = (
            "pygui-" + self.ep.libVersion().full
        )
    ...
    def updateBuddy(self, bud):
        ...
        if self.tv.exists(iid):
            ...
            self.tv.insert(
                str(bud.account.randId), "end", iid, open=True, text=text, values=values
            )
    ...
    def _onBuddyContextMenu(self, label):
        ...
        if label == "Audio call":
            ...
            if msgbox.askquestion("Delete buddy?", msg, default=msgbox.NO) != "yes":
                return
            iid = "buddy" + str(bud.randId)
            acc.buddyList.remove(bud)
            del bud
    ...
    def _onMenuSettings(self):
        ...
        if dlg.doModal():
            msgbox.showinfo(
                self.master.title(),
                "You need to restart for new settings to take effect",
            )

...
class Dialog(tk.Toplevel):
    ...
    def createBasicTab(self):
        ...
        self.cfgLogAppend = tk.BooleanVar(
            value=True
            if (self.cfg.epConfig.logConfig.fileFlags & pj.PJ_O_APPEND)
            else False
        )
        ...
        row = 0
        ttk.Label(frm, text="User Agent:").grid(
        ...
        row += 1
        ttk.Label(frm, text="Max calls:").grid(
            row=row, column=0, sticky=tk.E, pady=2, padx=8
        )
        ...
        row += 1
        ttk.Label(frm, text="Log file:").grid(
            row=row, column=0, sticky=tk.E, pady=2, padx=8
        )
    ...
    def createNetworkTab(self):
        ...
        self.cfgStunIgnoreError = tk.BooleanVar(
            value=self.cfg.epConfig.uaConfig.stunIgnoreFailure
        )
        ...
        self.cfgTlsVerifyClient = tk.BooleanVar(
            value=self.cfg.tls.config.tlsConfig.verifyClient
        )
        self.cfgTlsVerifyServer = tk.BooleanVar(
            value=self.cfg.tls.config.tlsConfig.verifyServer
        )
        ...
        ttk.Checkbutton(
            frm, text="Enable UDP transport", variable=self.cfgUdpEnabled
        ).grid(row=row, column=0, sticky=tk.W, padx=6, pady=2)
        row += 1
        ttk.Label(frm, text="UDP port:").grid(
            row=row, column=0, sticky=tk.E, pady=2, padx=8
        )
        tk.Spinbox(frm, from_=0, to=65535, textvariable=self.cfgUdpPort, width=5).grid(
            row=row, column=1, sticky=tk.W, padx=6
        )
        ttk.Label(frm, text="(0 for any)").grid(
        ...
        ttk.Checkbutton(
            frm, text="Enable TCP transport", variable=self.cfgTcpEnabled
        ).grid(row=row, column=0, sticky=tk.W, padx=6, pady=2)
        row += 1
        ttk.Label(frm, text="TCP port:").grid(
            row=row, column=0, sticky=tk.E, pady=2, padx=8
        )
        ...
        ttk.Checkbutton(
            frm, text="Enable TLS transport", variable=self.cfgTlsEnabled
        ).grid(row=row, column=0, sticky=tk.W, padx=6, pady=2)
        row += 1
        ttk.Label(frm, text="TLS port:").grid(
            row=row, column=0, sticky=tk.E, pady=2, padx=8
        )
        ...
        row += 1
        ttk.Label(frm, text="TLS CA file:").grid(
            row=row, column=0, sticky=tk.E, pady=2, padx=8
        )
        ...
        row += 1
        ttk.Label(frm, text="TLS cert file:").grid(
            row=row, column=0, sticky=tk.E, pady=2, padx=8
        )
        ...
        row += 1
        ttk.Checkbutton(
            frm, text="TLS verify server", variable=self.cfgTlsVerifyServer
        ).grid(row=row, column=1, sticky=tk.W, padx=6, pady=2)
        row += 1
        ttk.Checkbutton(
            frm, text="TLS verify client", variable=self.cfgTlsVerifyClient
        ).grid(row=row, column=1, sticky=tk.W, padx=6, pady=2)
        row += 1
        ttk.Label(frm, text="DNS and STUN:").grid(
            row=row, column=0, sticky=tk.W, pady=2, padx=8
        )
        row += 1
        ttk.Label(frm, text="Nameserver:").grid(
            row=row, column=0, sticky=tk.E, pady=2, padx=8
        )
        ...
        row += 1
        ttk.Label(frm, text="STUN Server:").grid(
            row=row, column=0, sticky=tk.E, pady=2, padx=8
        )
    ...
    def createMediaTab(self):
        ...
        self.cfgAudioPtime = tk.IntVar(
            value=self.cfg.epConfig.medConfig.audioFramePtime
        )
        ...
        frm.columnconfigure(1, weight=2)
        row = 0
        ttk.Label(frm, text="Max media ports:").grid(
            row=row, column=0, sticky=tk.E, pady=2, padx=8
        )
        ...
        row += 1
        ttk.Label(frm, text="Core clock rate:").grid(
            row=row, column=0, sticky=tk.E, pady=2, padx=8
        )
        tk.Spinbox(
            frm,
            ...
            to=48000,
            increment=8000,
            textvariable=self.cfgClockrate,
            width=5,
        ...
        tk.Spinbox(
            ...
            to=48000,
            increment=8000,
            textvariable=self.cfgSndClockrate,
            width=5,
        ).grid(row=row, column=1, sticky=tk.W, padx=6)
        ttk.Label(frm, text="(0: follow core)").grid(
            row=row, column=1, sticky=tk.E, pady=6, padx=6
        )
        row += 1
        ttk.Label(frm, text="Core ptime:").grid(
            row=row, column=0, sticky=tk.E, pady=2, padx=8
        )
        tk.Spinbox(
            frm,
            ...
            to=400,
            increment=10,
            textvariable=self.cfgAudioPtime,
            width=3,
        ).grid(row=row, column=1, sticky=tk.W, padx=6)
        row += 1
        ttk.Label(frm, text="RTP ptime:").grid(
            row=row, column=0, sticky=tk.E, pady=2, padx=8
        )
        tk.Spinbox(
            frm,
            ...
            to=400,
            increment=10,
            textvariable=self.cfgCodecPtime,
            width=3,
        ).grid(row=row, column=1, sticky=tk.W, padx=6)
        row += 1
        ...
        tk.Spinbox(
            frm, from_=1, to=10, textvariable=self.cfgMediaQuality, width=5
        ).grid(row=row, column=1, sticky=tk.W, padx=6)
        row += 1
    ...
    def onOk(self):
        ...
        self.cfg.epConfig.logConfig.fileFlags = (
            self.cfg.epConfig.logConfig.fileFlags | flags
        )

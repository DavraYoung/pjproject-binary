...
class TextFrame(ttk.Frame):
    ...
    def _createWidgets(self):
        ...
        self._text.tag_config(
            "info", foreground="darkgray", font=("Arial", "9", "italic")
        )
...
class AudioFrame(ttk.Labelframe):
    ...
    def _createInitWidgets(self):
        ...
        self._btnCancel = ttk.Button(
            self._initFrame, text="Cancel", command=self._onHangup
        )
        self._btnCancel.pack(side=tk.TOP)
    ...
    def _createWidgets(self):
        ...
        self.btnRxMute = ttk.Button(
            self.rxVolFrm, width=8, text="Mute", command=self._onRxMute
        )
        ...
        self.btnTxMute = ttk.Button(
            self.txVolFrm, width=8, text="Mute", command=self._onTxMute
        )
...
class ChatFrame(tk.Toplevel):
    ...
    def _createWidgets(self):
        ...
        btnText = ttk.Button(
            self.toolbar, text="Show/hide text", command=self._onShowHideText
        )
        btnText.pack(side=tk.LEFT, fill=tk.Y)
        btnAudio = ttk.Button(
            self.toolbar, text="Start/stop audio", command=self._onStartStopAudio
        )
        ...
        btnAdd = ttk.Button(
            self.toolbar, text="Add participant..", command=self._onAddParticipant
        )

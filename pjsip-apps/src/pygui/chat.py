...
class Chat(gui.ChatObserver):
    ...
    def _getCallFromUriStr(self, uri_str, op=""):
        ...
        if uri not in self._participantList:
            write(
                "=== " + op + " cannot find participant with URI '" + uri_str + "'\r\n"
            )
            return None
    ...
    def _getActiveMediaIdx(self, thecall):
        ...
        for mi in ci.media:
            if mi.type == pj.PJMEDIA_TYPE_AUDIO and (
                mi.status != pj.PJSUA_CALL_MEDIA_NONE
                and mi.status != pj.PJSUA_CALL_MEDIA_ERROR
            ):
                return mi.index
        return -1
    ...
    def updateCallState(self, thecall, info=None):
        ...
        if info.state < pj.PJSIP_INV_STATE_CONFIRMED:
            ...
            self.addMessage(
                None,
                "Call to '%s' disconnected: %s" % (thecall.peerUri, info.lastReason),
            )

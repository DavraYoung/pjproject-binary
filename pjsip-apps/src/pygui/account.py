...
class Account(pj.Account):
    ...
    def onIncomingCall(self, prm):
        ...
        if (
            msgbox.askquestion(
                msg, "Accept call from '%s'?" % (ci.remoteUri), default=msgbox.YES
            )
            == "yes"
        ):
    ...
    def onInstantMessageStatus(self, prm):
        ...
        chat.addMessage(
            None, "Failed sending message to '%s': %s" % (prm.toUri, prm.reason)
        )
    ...
    def onTypingIndication(self, prm):
        ...
        if not chat:
            write(
                "=== Incoming typing indication from "
                + prm.fromUri
                + "cannot find chat\r\n"
            )
            return
...
class AccountListFrame(ttk.Frame):
    ...
    def _createWidgets(self):
        self.tv = ttk.Treeview(
            self, columns=("ID", "Registrar", "Default"), selectmode="browse"
        )
        ...
        ttk.Button(self, text="Add..", command=self._onBtnAdd).grid(
            column=1, row=0, padx=5
        )
        ttk.Button(self, text="Settings..", command=self._onBtnSettings).grid(
            column=1, row=1
        )
        ttk.Button(self, text="Set Default", command=self._onBtnSetDefault).grid(
            column=1, row=2
        )
        ttk.Button(self, text="Delete..", command=self._onBtnDelete).grid(
            column=1, row=3
        )
    ...
    def _showAcc(self, acc):
        ...
        self.tv.insert(
            "", 0, str(acc.randId), open=True, text=str(acc.cfg.priority), values=values
        )
    ...
    def _onBtnDelete(self):
        ...
        if msgbox.askquestion("Delete account?", msg, default=msgbox.NO) != "yes":
            return
